...
public class QueryWikipediaForDescription extends AbstractDescriptionHandler {
    ...
    private static final String FILTER_FOR_NUTRITION = String.format("%s, %s, %s", H3_TAG, H2_TAG, P_TAG);
    ...
    private String getNutrition(String url) throws IOException {
        ...
        Elements newEles = wiki.select(FILTER_FOR_NUTRITION);

        // Build a string of all the nutrient descriptions for that ingredient
        String nutrientDescriptions = newEles.parallelStream().filter(filterPredicate)
                .map(element -> ((Element) element.nextSibling()).text())
                .collect(Collectors.joining("\n"));

        // Return what was found (will default to the default nutrition description if nothing was found).
        return (nutrientDescriptions == null || nutrientDescriptions.trim() == "") ? 
                Ingredient.DEFAULT_NUTRITION_DESCRIPTION : nutrientDescriptions;
    }
    ...
    private String lookupIngredient(String ingredient) throws IOException {
        ...
        String url = String.format(BASE_WIKIPEDIA_SEARCH_URL, 
                ingredient.replaceAll(" ", "%20"));
        ...
        return regexMatcher.find() ? String.format(WIKIPEDIA_ARTICLE_LOOKUP_URL, regexMatcher.group(1)) 
                : null;
    }
